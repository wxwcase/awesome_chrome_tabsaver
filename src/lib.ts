...
// domain, and the group holds no other tabs. Returns only the domains that
// still need (re)grouping, alphabetically.
export function domainGroupsToApply(
  ...
  for (const domain of [...byDomain.keys()].sort((a, b) =>
    ...
    const satisfied =
      ...
      titleById.get(onlyGroup) === domain &&
      memberCount.get(onlyGroup) === ids.length;
    if (!satisfied) result.push({ domain, tabIds: ids });
  }
  return result;
}
